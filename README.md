...
`Can't access your Google Account`（`We can't access this content right now.
Try signing into your Google account or allowing cookie access to proceed.`）
と表示されることがあります。これはブラウザのCookie制限によりGoogleの
ログインポップアップが正しく動作しないことが原因です。

**iPhone (Safari) の場合:**

1. 「設定」アプリ → 「Safari」→ 「プライバシーとセキュリティ」（iOSのバージョンに
   よっては「詳細」の場合もあります）を開く
2. 「サイト越えトラッキングを防ぐ」をOFFにする
3. Safariを一度完全に終了し、アプリを開き直してから再度試す

この設定はSafari全体に影響するため、気になる場合はDrive連携を使う時だけ
一時的にOFFにし、終わったら再度ONに戻しても構いません。

**Androidの場合:**

同様の現象が起きる場合は、Chromeの「サードパーティ Cookie」のブロック設定が
原因の可能性があります。Chromeアプリ → 右上のメニュー →「設定」→
「プライバシーとセキュリティ」→「サードパーティ Cookie」で、ブロックが
有効になっていないか確認してください。

**共通の注意点:**

LINEやSNSアプリなどのリンクから開いた「アプリ内ブラウザ」では、Googleが
セキュリティ上の理由でログインそのものを明示的にブロックしています。
その場合は共有メニュー等から「Safariで開く」「Chromeで開く」を選び、
標準のブラウザで開き直してください。

なお、これはあくまでDrive自動アップロード（任意機能）に関する制限で、
領収書のOCR読み取り → エクセル明細作成・ダウンロードという中核機能には
影響ありません。
...
- 確定申告での提出は「医療費控除の明細書」のみで足りますが、**領収書の原本は
  確定申告期限から5年間、自宅等で保管する義務があります**（税務署から提示を
  求められる場合があるため）。本アプリのGoogle Drive保存機能はあくまで
  控え・バックアップであり、原本の保管に代わるものではありません。紙の
  領収書は処分せずに保管してください
